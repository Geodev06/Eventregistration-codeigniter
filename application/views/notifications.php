        ...
        <div class="p-5" id="dash_panel">
            ...
            <div class="row">
                ...
                <?php
                    ...
                    $qry ="SELECT user_id,content,res_action,date_created,sender,acronym from tbl_notifications where user_id ='".$_SESSION["user_id"]."' ORDER BY date_created DESC";
                    $result = mysqli_query($db, $qry);
                    if ($result) {
                    while($row = mysqli_fetch_assoc($result)) {

                        echo "<div class='alert alert-secondary border-0 '>";
                        echo "<strong>Notifications!</strong>";
                        echo "<p id ='avatar_sm'>".$row["acronym"]."</p> ";
                        echo "<h6>".$row["content"]."</h6><p style='font-size: 11px; margin:0px'> Date : ".$row["date_created"]."</p>";
                        echo "<form action=". base_url().'index.php/dashboard/search/read'." method='post'>";
                        echo "<input type='hidden' value='".$row['sender']."' name='txtsearch'";
                        echo "<br><br><button type='submit' class='btn-view'>View</button>";
                        echo "</form>";
                        echo "<p style='margin-top:5px'> Notification status : ".$row["res_action"]."</p>";
                        echo "</div>";
                    }
                    } else {
